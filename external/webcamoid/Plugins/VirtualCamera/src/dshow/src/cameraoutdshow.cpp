...
{
    Q_UNUSED(password)
    qDebug() << "driverPath is " << m_driverPath;
    ...
    QString params =
            QString("/c \"regsvr32 /s \"%1\" && reg add %2 /v FriendlyName /d \"%3\" /f\"")
            .arg(this->m_driverPath)
            .arg(reg)
            .arg(desc);

    qDebug() << "Full command is : " << params;
...
bool CameraOutDShow::init(int streamIndex, const AkCaps &caps)
{
    ...
    this->m_caps = caps;
    bool retval = this->m_ipcBridge.open(IPC_FILE_NAME, IpcBridge::Write);
    qDebug() << "IPC BRIDGE OPEN "<<retval;
    return retval;
}
