...
private slots:
    ...
    void MOVEComUploadProgress(UploadingFile* file);
    void MOVEComUploadCompleted(UploadingFile* file);
    void MOVEComTransferAborted(TransferringFile *file);
    void MOVEComReady(bool ready);

    void transferDialogCompleted();
    void transferDialogAbortRequested();

    void MOVEComDeleteOK(QString path, int id);
    void MOVEComPostOK(QString path);
